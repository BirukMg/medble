<template>
    <div>
      ...
      <div class="container">
        ...
        <div class="row">
        <div class="col-6">
          <h6 style="float: left" class="font-weight-bold">{{$t('post.category')}}: <span class="text-secondary font-weight-bold text-capitalize">{{$t('category.houses.' + 'houses'.toUpperCase())}} ({{$t('category.houses.' + postData.category.toLowerCase())}})</span></h6>
          ...
          <div class="form-group">
            ...
            <select id="category" class="browser-default custom-select custom-select-sm" v-model="postData.category">
              <option v-for="(i,index) in houses" :key="i" :value="houses[index]">{{$t('category.houses.' + i.toLowerCase())}}</option>
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label style="float: left">{{$t('post.condition')}}</label>
              <select class="custom-select custom-select-sm" v-model="postData.condition">
                <option value="New">{{$t('post.house.condition.New')}}</option>
                <option value="On construction">{{$t('post.house.condition.On construction')}}</option>
                <option value="Almost finished">{{$t('post.house.condition.Almost finished')}}</option>
                <option value="Finished">{{$t('post.house.condition.Finished')}}</option>
                <option value="Old">{{$t('post.house.condition.Old')}}</option>
              </select>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.bedroom')" outline size="sm" v-model="houseData.bedroom"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.bathroom')" outline size="sm"  v-model="houseData.bathroom"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.kitchin')" outline size="sm"  v-model="houseData.kitchin"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.square_footage')" outline size="sm"  v-model="houseData.square_footage"/>
          </div>
         <mdb-btn-group>
...
export default {
    ...
    data(){
        return{
            ...
            houses: ["real estate","condominium","duplexes","homes","land","apartements","rooms for rent","short term rentals","vacation rentals","others"],
        }
    },
    mounted() {
      this.init()
    ...
    methods: {
      init(){
        ...
        .then(res => {
          ...
          this.houseData = Object.assign({}, res.data.houses)
          delete this.postData.houses
      ...
      update(){
        this.checkEmptyVec(this.houseData)
        ...
        if (this.passVec && this.passPost) {
          if (this.addImgData.selectedImg.length == 0) {
            ...
            axios.post(this.$store.state.server_ip +'/api/houses/edithouse',{
      ...
      checkEmpty(object){
          for (const key in object) {
            if (object.hasOwnProperty(key)) {
              if (object[key] == '' && key != 'view' && key != 'img' && key != 'post_date') {
                this.snackbar.snackbar = true
          ...
          for (const key in object) {
            if (object.hasOwnProperty(key)) {
              if (object[key] == '' && key != 'view') {
                this.snackbar.snackbar = true
                this.snackbar.text =this.$t('post.house.' + key.replace(/_/g, '') + "req")
                this.snackbar.color = 'red darken-2'
                this.snackbar.timeout = 3000
                this.passVec = false
                break
