    ...
    <div>
      ...
      <div class="container">
       <v-snackbar v-model="snackbar.snackbar" :top="true" style="margin-top: 70px" :color="snackbar.color">
        ...
        <div class="col-2"></div>
        <div class="col-8">
          <h6 style="float: left" class="font-weight-bold">{{$t('post.category')}}: <span class="text-secondary font-weight-bold text-capitalize">{{$t('category.reviews.' + 'reviews'.toUpperCase())}} ({{$t('category.reviews.' + postData.category.toLowerCase())}})</span></h6>
          ...
          <div class="form-group">
            ...
            <select id="category" class="browser-default custom-select custom-select-sm" v-model="postData.category">
              <option v-for="(i,index) in reviews" :key="i" :value="reviews[index]">{{$t('category.reviews.' + i.toLowerCase())}}</option>
            </select>
          </div>
          <div class="form-group">
...
export default {
    ...
    data(){
        return {
            ...
            reviews:["bank","bar","cafe","clinic","hospitals","hotels","mall","restaurants","salons","schools","supermarkets","transport","others"]
        }
    },
    mounted() {
