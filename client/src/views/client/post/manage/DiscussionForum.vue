    ...
    <div>
      ...
      <div class="container">
       <v-snackbar v-model="snackbar.snackbar" :top="true" style="margin-top: 70px" :color="snackbar.color">
        ...
        <div class="col-2"></div>
        <div class="col-8">
          <h6 style="float: left" class="font-weight-bold">{{$t('post.category')}}: <span class="text-secondary font-weight-bold text-capitalize">{{$t('category.discussion forum.' + 'discussion forum'.toUpperCase())}} ({{$t('category.discussion forum.' + postData.category.toLowerCase())}})</span></h6>
          ...
          <div class="form-group">
            ...
            <select id="category" class="browser-default custom-select custom-select-sm" v-model="postData.category">
              <option v-for="(i,index) in discussionForum" :key="i" :value="discussionForum[index]">{{$t('category.discussion forum.' + i.toLowerCase())}}</option>
            </select>
          </div>
          <div class="form-group">
...
export default {
    ...
    data(){
        return {
            ...
            discussionForum:["mobile phones","arts","autos","beauty","bikes","business","entertainment","fix it","food","gaming","garden","health","history","housing","legal","marriage","money","music","pets","philos",
            "photo","politics","science","spirit","transport","writing"]
        }
    },
    mounted() {
