<template>
    <div>
      ...
      <div class="container">
        ...
        <div class="row">
        <div class="col-6">
          <h6 style="float: left">{{$t('post.category')}}: <span class="text-secondary font-weight-bold">{{$t('category.houses.' + 'houses'.toUpperCase())}} ({{$t('category.houses.' + item.toLowerCase())}})</span></h6>
          ...
          <div class="form-group">
            <label for="state" style="float: left">{{$t('post.Select category')}}</label>
        ...
        <div class="col-6">
          ...
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.bedroom')" outline size="sm" v-model="homeData.bedroom"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.bathroom')" outline size="sm"  v-model="homeData.bathroom"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.kitchin')" outline size="sm"  v-model="homeData.kitchin"/>
          </div>
          <div class="form-group">
            <mdb-input type="text" :label="$t('post.house.square_footage')" outline size="sm"  v-model="homeData.square_footage"/>
          </div>
         <mdb-btn-group>
...
export default {
    ...
    methods: {
      ...
      post(){
        this.checkEmptyVec(this.homeData)
        ...
        if (this.passVec && this.passPost) {
          if (JSON.parse(this.$store.state.account).number_of_posts > this.$store.state.post_limit && JSON.parse(this.$store.state.account).account_type == 'free') {
            ...
            if (this.addImgData.selectedImg.length <= 0) {
              ...
              axios.post(this.$store.state.server_ip +'/api/houses/posthome',{postData: this.postData, homeData: this.homeData})
              .then(res=> {
